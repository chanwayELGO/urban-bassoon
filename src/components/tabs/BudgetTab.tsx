...
import { compressImage, isImage, processFiles } from "../../lib/attachments"
import { CURRENCIES, EXPENSE_CATS, countryFlag } from "../../lib/constants"
...
export function BudgetTab({
  ...
  const personColor = (name) =>
    PALETTE[Math.abs([...name].reduce((h, c) => h * 31 + c.charCodeAt(0), 0)) % PALETTE.length]
  ...
  const getRate = async (from, to) => {
    ...
    try {
      const data = await fetch(`https://api.frankfurter.app/latest?from=${from}&to=${to}`).then(
        (r) => r.json(),
      )
  ...
  const barColor =
    pct > 85
      ? "linear-gradient(90deg,#FF5C5C,#C93030)"
      : pct > 60
        ? "linear-gradient(90deg,#FFB347,#E07C00)"
        : "linear-gradient(90deg,#4DB87A,#2D9E5A)"
  const catBreakdown = EXPENSE_CATS.map((cat) => ({
    cat,
    total: expenses
      .filter((e) => e.cat === cat)
      .reduce((s, e) => s + (e.amtBase ?? Number(e.amount)), 0),
  }))
    .filter((c) => c.total > 0)
  ...
  const balanceNet = {}
  const initP = (n) => {
    if (balanceNet[n] === undefined) balanceNet[n] = 0
  }
  ...
  const simplifyDebts = () => {
    const creditors = Object.entries(balanceNet)
      .filter(([, v]) => v > 0)
      .map(([n, v]) => ({ n, v }))
    const debtors = Object.entries(balanceNet)
      .filter(([, v]) => v < 0)
      .map(([n, v]) => ({ n, v: -v }))
    const txns = []
    let ci = 0,
  ...
  const fmtOrig = (exp) =>
    exp.currency !== baseCurrency
      ? `${exp.currency} ${Number(exp.originalAmount).toFixed(2)}`
      : null
  const myShare = (exp) => {
    if (!exp.splits) return exp.amtBase ?? Number(exp.amount)
    const me = exp.splits.find((s) => s.name === "Me")
    return me ? me.amtBase : 0
  }
...
If you can't read a field clearly, make your best inference and lower confidence. Return ONLY the JSON object.`,
              messages: [
                {
                  ...
                  content: [
                    ...
                    {
                      type: "text",
                      text: "Extract all receipt data from this image and return the JSON.",
                    },
                  ],
                },
              ],
  ...
  const confClass = (c) =>
    c === "high" ? "ocr-conf-high" : c === "medium" ? "ocr-conf-mid" : "ocr-conf-low"

  // Attach a receipt to an expense
  const addReceiptToExp = (expId, attachments) => {
    saveExpenses(
      expenses.map((e) =>
        e.id === expId ? { ...e, attachments: [...(e.attachments || []), ...attachments] } : e,
      ),
    )
  ...
  const Avatar = ({ name, size = 26 }) => (
    <div
      style={{
        ...
        height: size,
        borderRadius: "50%",
        background: name === "Me" ? "rgba(255,168,40,0.2)" : personColor(name) + "22",
        color: name === "Me" ? "var(--amber)" : personColor(name),
        display: "flex",
        alignItems: "center",
  ...
  return (
    <div>
      ...
      <div className="card">
          ...
          <div className="flex gap-6 items-center">
            ...
            {!editingBudget ? (
              ...
              <div className="flex gap-6">
                <input
                  className="input"
                  style={{ width: 88, padding: "5px 8px", fontSize: 12 }}
                  value={budgetInput}
        ...
        <div className="grid-3">
          <div className="stat-card">
            <div className="stat-val" style={{ fontSize: 17 }}>
              {baseCurrency} {totalBudget.toLocaleString()}
            </div>
          ...
          <div className="stat-card">
            <div
              className="stat-val"
              style={{ fontSize: 17, color: over ? "#FF5C5C" : "var(--amber)" }}
            >
              {baseCurrency} {spent.toFixed(0)}
            </div>
            <div className="stat-lbl">Spent</div>
          </div>
          <div className="stat-card">
            <div className="stat-val" style={{ fontSize: 17, color: over ? "#FF5C5C" : "#4DB87A" }}>
              {over ? "-" : ""}
              {baseCurrency} {Math.abs(remaining).toFixed(0)}
            </div>
            <div className="stat-lbl">{over ? "Over!" : "Left"}</div>
          </div>
        </div>
        ...
        >
          <div
            className="geo-toggle-row"
            style={{ flex: 1, padding: 0 }}
            onClick={() => saveGeoDetectEnabled(!geoDetectEnabled)}
          >
            <div className={`toggle-track${geoDetectEnabled ? " on" : ""}`}>
          ...
          {geoDetectEnabled && (
            <button className="geo-detect-btn" disabled={geoChecking} onClick={() => onDetectNow()}>
              {geoChecking ? (
                <span className="dots">
                  <span>·</span>
                  <span>·</span>
      ...
      <div className="card">
        ...
        <div className="flex gap-8">
          ...
          />
          <button className="btn-ghost" onClick={handleAddPerson}>
            Add
          </button>
        </div>
      </div>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              color: activeTab === id ? "#07101F" : "rgba(230,217,194,0.5)",
        ...
        <div className="card">
          <div className="flex items-center justify-between" style={{ marginBottom: 12 }}>
            ...
            {(ocrState.step === "compressing" || ocrState.step === "analyzing") && (
              <div
                className="ai-gen-btn"
                style={{ fontSize: 11, opacity: 0.7, pointerEvents: "none" }}
              >
                <span className="dots">
                  <span>·</span>
                  <span>·</span>
          ...
          {ocrState.step === "idle" && !form.desc && !form.amount && (
            ...
            >
              ...
              <div className="ocr-scan-sub">
                Claude reads the total, merchant, date & category automatically
              </div>
            </div>
          )}
          ...
          {ocrState.step === "ready" && ocrState.result && (
            <div className="ocr-result" style={{ marginBottom: 14 }}>
              {ocrState.image && (
                <img className="ocr-result-img" src={ocrState.image} alt="Receipt" />
              )}
              <div className="ocr-result-body">
                ...
                {[
                  ...
                  {
                    label: "Total",
                    val: ocrState.result.amount
                      ? `${ocrState.result.currency || ""} ${ocrState.result.amount}`
                      : null,
                  },
                ...
                {ocrState.result.items?.length > 0 && (
                  <div className="ocr-items-list">
                    ...
                    {ocrState.result.items.length > 5 &&
                      ` +${ocrState.result.items.length - 5} more`}
                  </div>
                )}
              ...
              <div className="ocr-result-footer">
                <button className="btn flex-1" onClick={acceptOcr}>
                  Use this ✓
                </button>
                <button
                  className="btn-ghost"
                  style={{ fontSize: 12 }}
                  onClick={() => setOcrState((s) => ({ ...s, step: "idle", result: null }))}
                >
                  Edit manually
                </button>
              </div>
            ...
            <div style={{ position: "relative", marginBottom: 10 }}>
              ...
              <div style={{ position: "absolute", top: 6, right: 6 }}>
                <button
                  ...
                  onClick={() => setForm((f) => ({ ...f, receiptImage: null }))}
                >
                  ✕
                </button>
              </div>
          ...
          <div className="flex flex-col gap-8">
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {allPeople.map((p) => (
                  <div
                    key={p}
                    className={`person-chip${form.paidBy === p ? " on" : ""}`}
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 16,
                        borderRadius: "50%",
                        background: p === "Me" ? "var(--amber)" : personColor(p),
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        fontSize: 8,
              ...
              <>
                <div
                  ...
                  onClick={() =>
                    setForm((f) => ({
                      ...f,
                      splitEnabled: !f.splitEnabled,
                      splitWith: f.splitEnabled ? [] : allPeople.filter((n) => n !== f.paidBy),
                    }))
                  }
                >
                  <div className={`toggle-track${form.splitEnabled ? " on" : ""}`}>
                    <div className="toggle-knob" />
                  </div>
                ...
                {form.splitEnabled && (
                  <div>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                      {allPeople
                        .filter((n) => n !== form.paidBy)
                        .map((p) => (
                          <div
                            key={p}
                            className={`person-chip${form.splitWith.includes(p) ? " on" : ""}`}
                    ...
                    {form.splitWith.length > 0 &&
                      (() => {
                        const participants = [...new Set([form.paidBy, ...form.splitWith])]
                        const base = convertedPreview
                          ? convertedPreview.amtBase
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              Equal split — {participants.length} people · {fmt(share)} each
                            </div>
                            {participants.map((p) => (
            ...
            <button
              ...
              onClick={addExpense}
              disabled={
                !form.desc.trim() || !form.amount || converting || ocrState.step === "analyzing"
              }
            >
      ...
      {activeTab === "transactions" &&
        (() => {
          ...
          const isToday = (exp) => {
            ...
            try {
              return new Date(exp.date).toISOString().split("T")[0] === todayISO
            } catch {
              return false
            }
          }
            ...
            <div>
                  ...
                  {catBreakdown.map(({ cat, total }) => (
                    <div key={cat} style={{ marginBottom: 10 }}>
                      ...
                      <div className="prog-track" style={{ height: 4 }}>
                        <div
                          className="prog-fill"
                          style={{ width: `${Math.min(100, (total / spent) * 100)}%` }}
                        />
                      </div>
                    </div>
                  ))}
              ...
              <div className="card">
                ...
                <div className="gallery-filters">
                  {[
                    ...
                    <div
                      key={f.id}
                      className={`gf-chip${galleryFilter === f.id ? " on" : ""}`}
                      onClick={() => setGalleryFilter(f.id)}
                    >
                      {f.label}
                      {f.id === "all" && <span style={{ opacity: 0.5 }}>({expenses.length})</span>}
                      {f.id === "photos" && (
                        <span style={{ opacity: 0.5 }}>
                          ({expenses.filter((e) => firstImg(e)).length})
                        </span>
                      )}
                    </div>
                  ))}
                ...
                {viewMode === "list" &&
                  (sortedExps.length === 0 ? (
                    ...
                    sortedExps.map((exp) => {
                      const orig = fmtOrig(exp)
                      const isSplit = !!exp.splits
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              <div
                                style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 3 }}
                              >
                                {orig && <span className="curr-badge">{exp.currency}</span>}
                                {isSplit && (
                                  <span className="exp-split-tag">
                                    👥 Split {exp.splits.length}
                                  </span>
                                )}
                                {exp.paidBy && exp.paidBy !== "Me" && (
                                  <span className="payer-tag">💳 {exp.paidBy}</span>
                                )}
                                {atts.length > 0 && (
                                  <span
                                    style={{
                                      fontSize: 10,
                                      color: "var(--amber)",
                                      background: "rgba(255,168,40,0.08)",
                                      border: "1px solid rgba(255,168,40,0.2)",
                                      borderRadius: 5,
                                      padding: "1px 6px",
                                    }}
                            ...
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              ...
                              {isSplit && (
                                <div style={{ fontSize: 10, color: "var(--text-dim)" }}>
                                  your share: {fmt(share)}
                                </div>
                              )}
                              {orig && (
                                <div style={{ fontSize: 10, color: "var(--text-dim)" }}>{orig}</div>
                              )}
                            </div>
                            <button
                              className="btn-del"
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="gallery-cell-overlay">
                            <div className="gallery-cell-amount">
                              {fmt(exp.amtBase ?? exp.amount)}
                            </div>
              ...
              {selectedExp && (
                <div
                  ...
                  onClick={(e) => e.target === e.currentTarget && setSelectedExp(null)}
                >
                  <div className="exp-detail-sheet">
                    ...
                    {(() => {
                      ...
                      return img ? (
                        <img
                          src={img.data}
                          className="exp-detail-photo"
                          alt="Receipt"
                          onClick={() => setViewingAtt(img)}
                        />
                      ) : (
                    ...
                    <div className="exp-detail-body">
                      ...
                      <div className="exp-detail-row">
                        ...
                        <div
                          className="exp-detail-val"
                          style={{ color: "var(--amber)", fontWeight: 700, fontSize: 16 }}
                        >
                      ...
                      <div className="exp-detail-row">
                        ...
                        <div className="exp-detail-val">
                          {selectedExp.cat?.split(" ").slice(1).join(" ") || "Other"}
                        </div>
                      </div>
                      ...
                      <div className="exp-detail-row">
                        ...
                        <div className="exp-detail-val">
                          {selectedExp.date}
                          {isToday(selectedExp) && (
                            <span className="exp-split-tag" style={{ marginLeft: 6 }}>
                              Today
                            </span>
                          )}
                        </div>
                      ...
                      {selectedExp.activityText && (
                        <div className="exp-detail-row">
                          ...
                          <div className="exp-detail-val">
                            {selectedExp.dayLabel} — {selectedExp.activityText}
                          </div>
                        </div>
                      )}
                      ...
                      {(selectedExp.attachments || []).length > 0 && (
                        <div style={{ marginTop: 14 }}>
                          <div className="card-title-small" style={{ marginBottom: 8 }}>
                            Attachments
                          </div>
                          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                            ...
                            >
                              ...
                              <input
                                ...
                                onChange={async (e) => {
                                  const p = await processFiles(e.target.files)
                                  if (p.length) {
                                    addReceiptToExp(selectedExp.id, p)
                                    setSelectedExp((s) => ({
                                      ...s,
                                      attachments: [...(s.attachments || []), ...p],
                                    }))
                                  }
                                  e.target.value = ""
                                }}
                    ...
                    <div className="exp-detail-actions">
                      {firstImg(selectedExp) && (
                        <button
                          ...
                          onClick={() => setViewingAtt(firstImg(selectedExp))}
                        >
                          🔍 Full Receipt
                        </button>
                      )}
                      <button
                        className="btn-ghost flex-1"
                        style={{ borderColor: "rgba(255,92,92,.3)", color: "#FF5C5C" }}
                        onClick={() => {
                          saveExpenses(expenses.filter((e) => e.id !== selectedExp.id))
                          setSelectedExp(null)
                        }}
                      >
                        🗑 Delete
                      </button>
                      <button className="btn flex-1" onClick={() => setSelectedExp(null)}>
                        Done
                      </button>
                    </div>
                  </div>
      ...
      {activeTab === "balances" && (
        <div>
          {people.length === 0 ? (
            <div className="empty">
              <div className="empty-icon">👥</div>
              <div className="empty-msg">Add travellers above to track who owes what</div>
            </div>
          ) : (
            <>
              <div className="card">
                  ...
                  return (
                    <div key={name} className="balance-row">
                      ...
                      {abs < 0.005 ? (
                        ...
                        <span className="balance-owed">+{fmt(abs)} owed to them</span>
                      ) : (
              ...
              <div className="card">
                ...
                {allPeople.map((name) => {
                  const total = expenses.reduce((s, exp) => {
                    if (!exp.splits)
                      return exp.paidBy === name ? s + (exp.amtBase ?? Number(exp.amount)) : s
                    ...
                    <div key={name} style={{ marginBottom: 12 }}>
                      ...
                      <div className="prog-track" style={{ height: 5 }}>
                        <div
                          className="prog-fill"
                          style={{
                            width: `${spent > 0 ? Math.min(100, (total / spent) * 100) : 0}%`,
                          }}
                        />
                      </div>
                    </div>
      ...
      {activeTab === "daily" &&
        (() => {
          ...
          const days = Array.from({ length: 7 }, (_, i) => {
            ...
            const dayExps = expenses.filter(
              (e) =>
                ...
                (() => {
                  try {
                    return new Date(e.date).toISOString().split("T")[0] === iso
                  } catch {
                    return false
                  }
                })(),
          ...
          const todayPct =
            dailyBudget > 0 ? Math.min(100, Math.round((todaySpent / dailyBudget) * 100)) : 0
          ...
          return (
            <div>
              ...
              {notifPerm !== "granted" && (
                <div className="notif-perm-card" style={{ marginBottom: 12 }}>
                  ...
                  <div className="notif-perm-body">
                    ...
                    <div className="notif-perm-sub">
                      Get push notifications at 80% and 100% of your daily limit — even when the app
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (r === "granted") setActiveTab("daily")
                      }}
                    >
                      {notifPerm === "denied"
              ...
              <div className="card">
                ...
                <div className="flex gap-8">
                  <input
                    className="input flex-1"
                    type="number"
                    placeholder={`e.g. 80 (${baseCurrency})`}
                    value={dailyBudget || ""}
                    onChange={(e) => saveDailyBudget(Number(e.target.value) || 0)}
                  />
                  <div
                    style={{
                      display: "flex",
                ...
                {dailyBudget > 0 && (
                  <div style={{ marginTop: 10, fontSize: 11, color: "var(--text-dim)" }}>
                    = {baseCurrency} {(dailyBudget * 7).toFixed(0)}/week · {baseCurrency}{" "}
                    {(dailyBudget * 30).toFixed(0)}/month
                  </div>
                )}
              </div>
                ...
                <div className="card">
                  ...
                  <div className="daily-stat-row">
                    <div className="daily-chip">
                      <div className="daily-chip-val" style={{ color: barColor }}>
                        {baseCurrency} {todaySpent.toFixed(2)}
                      </div>
                      <div className="daily-chip-lbl">Spent</div>
                    </div>
                    <div className="daily-chip">
                      <div className="daily-chip-val">
                        {baseCurrency} {dailyBudget.toFixed(2)}
                      </div>
                      <div className="daily-chip-lbl">Limit</div>
                    </div>
                    <div className="daily-chip">
                      <div
                        className="daily-chip-val"
                        style={{ color: todayOver ? "#FF5C5C" : "#4DB87A" }}
                      >
                        {todayOver ? "-" : ""}
                        {baseCurrency} {Math.abs(dailyBudget - todaySpent).toFixed(2)}
                      </div>
                      ...
                      style={{
                        ...
                        background:
                          todayPct > 100
                            ? "linear-gradient(90deg,#FF5C5C,#C93030)"
                            : todayPct > 80
                              ? "linear-gradient(90deg,#FFA828,#E07C00)"
                              : "linear-gradient(90deg,#4DB87A,#2D9E5A)",
                      }}
                    />
                  </div>
                  ...
                  >
                    <span>0</span>
                    <span style={{ color: "rgba(255,168,40,.5)" }}>80%</span>
                    <span>
                      {baseCurrency} {dailyBudget}
                    </span>
                  </div>
                      ...
                      <div style={{ marginTop: 12 }}>
                        ...
                        {expenses
                          ...
                          .map((exp) => (
                            ...
                            >
                              <div className="exp-cat" style={{ fontSize: 17 }}>
                                {exp.cat.split(" ")[0]}
                              </div>
              ...
              <div className="card">
                ...
                {dailyBudget > 0 && (
                  <div style={{ fontSize: 11, color: "rgba(255,168,40,.5)", marginBottom: 2 }}>
                ...
                <div className="seven-day-chart" style={{ position: "relative" }}>
                  ...
                  {days.map((day, i) => {
                    ...
                    const col = over ? "#FF5C5C" : pct > 0.8 ? "#FFA828" : "#4DB87A"
                    return (
                      <div key={day.iso} className="day-bar-wrap">
                        {day.spent > 0 && (
                          <div className="day-bar-val">
                            {day.spent < 10 ? day.spent.toFixed(1) : Math.round(day.spent)}
                          </div>
                        )}
                        <div
                          ...
                          style={{
                            ...
                            background: day.isToday
                              ? `linear-gradient(to top,${col},${col}aa)`
                              : col,
                            opacity: day.isToday ? 1 : 0.6,
                          }}
                        />
                        <div className={`day-bar-label${day.isToday ? " today" : ""}`}>
                          {day.label}
                        </div>
                      </div>
                    )
...
export const AIRLINES = {
  ...
  AM: {
    name: "Aeroméxico",
    web: "https://aeromexico.com/en-us/travel-information/manage-your-trip",
  },
  ...
  PR: {
    ...
    web: "https://philippineairlines.com/en/ph/home/manage-booking",
  },
  ...
  SN: {
    ...
    web: "https://brusselsairlines.com/en/your-trip/manage-booking",
  },
  SQ: {
    name: "Singapore Airlines",
    web: "https://singaporeair.com/en_UK/us/travel-info/manage-booking",
  },
