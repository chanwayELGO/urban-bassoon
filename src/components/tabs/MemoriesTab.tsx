...
Rules: title should feel like a chapter heading, not a description. mood must match the visual atmosphere. notes should make the reader feel present.`,
                  ...
                  content: [
                    {
                      ...
                      source: {
                        ...
                        data: base64jpeg.split(",")[1],
                      },
                    },
                    {
                      type: "text",
                      text: "Analyse this travel photo and return the caption JSON.",
                    },
                  ],
                },
              ],
      ...
      setPhotoCaption({
        loading: false,
        result: null,
        error: "Couldn't read the photo. You can still caption it manually.",
      })
    }
  }
...
Generate a rich memory entry.`,
      ...
      setGenSuggestion({
        notes: "Could not generate — check your connection.",
        mood: form.mood,
        title: form.title,
      })
    }
...
Write an enhanced journal entry for this memory.`,
      ...
      setEnhancing((p) => ({
        ...p,
        [mem.id]: { loading: false, result: "⚠️ Could not enhance. Try again." },
      }))
    }
  }
  ...
  const dismissEnhancement = (id) =>
    setEnhancing((p) => ({ ...p, [id]: { loading: false, result: null } }))
  ...
  const generateStory = async () => {
    ...
    setShowStory(true)
    try {
      const memSummaries = [...memories]
        .sort((a, b) => a.date.localeCompare(b.date))
        .map(
          (m) =>
            `• ${m.date} | ${m.mood} ${m.title} @ ${m.locationName || "unknown location"}: ${m.notes || "(no notes)"}`,
        )
        .join("\n")
...
Write the travel story.`,
      ...
      setStory({
        loading: false,
        text: "⚠️ Could not generate story. Check your connection and try again.",
      })
    }
  }
  ...
  useEffect(() => {
    ...
    map.on("click", (e) => {
      ...
      const pIcon = L.divIcon({
        className: "",
        html: `<div style="width:22px;height:22px;background:#FFA828;border:3px solid #fff;border-radius:50%;box-shadow:0 0 0 4px rgba(255,168,40,0.3);animation:pulse 1s infinite;"></div>`,
        iconSize: [22, 22],
        iconAnchor: [11, 11],
      })
      ...
      fetch(
        ...
        .then((d) => {
          const loc = d.address
            ? [d.address.city || d.address.town || d.address.village, d.address.country]
                .filter(Boolean)
                .join(", ")
            : ""
        ...
        .catch(() => {
          setPinForm({ lat, lng, locationName: "" })
          setForm((f) => ({
            ...f,
            locationName: "",
            title: "",
  ...
  useEffect(() => {
    ...
    memories.forEach((mem) => {
      ...
      m.bindPopup(
        `<div class="popup-title">${mem.mood} ${mem.title}</div><div class="popup-meta">📍 ${mem.locationName || ""}${mem.locationName ? " · " : ""}${mem.date}</div>${mem.notes ? `<div class="popup-notes">${mem.notes.slice(0, 120)}${mem.notes.length > 120 ? "…" : ""}</div>` : ""}`,
      )
    ...
    <div>
      ...
      {memories.length === 0 ? (
        <div className="empty" style={{ paddingTop: 24 }}>
          ...
          <div className="empty-msg">
            Tap the map to drop your first memory pin.
            <br />
              ...
              return (
                <div key={mem.id} className="memory-card">
                  <div className="mem-header">
                    ...
                    <button className="btn-del" onClick={() => deleteMemory(mem.id)}>
                      ✕
                    </button>
                  </div>
                  ...
                  {enh.result && (
                    <div>
                      ...
                      <div className="enhance-actions">
                        <button className="btn btn-sm" onClick={() => acceptEnhancement(mem)}>
                          Accept & Replace
                        </button>
                        <button
                          className="btn-ghost"
                          style={{ fontSize: 11 }}
                          onClick={() => dismissEnhancement(mem.id)}
                        >
                          Discard
                        </button>
                      </div>
                  ...
                  <div className="mem-coords">
                    <span>🌐</span>
                    <span>
                      {mem.lat.toFixed(4)}°, {mem.lng.toFixed(4)}°
                    </span>
                    <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
                      <button
                        className="ai-gen-btn"
                        disabled={enh.loading}
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (leafletMap.current)
                            leafletMap.current.flyTo([mem.lat, mem.lng], 13, { duration: 1.2 })
          ...
          <div className="pin-form" style={{ overflowY: "auto", maxHeight: "90vh" }}>
            ...
            >
              <div style={{ fontSize: 12, color: "var(--text-dim)" }}>
                {form.locationName || form.title
                  ? `📍 ${form.locationName || form.title}`
                  : "Drop a pin to get started"}
              </div>
              <button
                className="ai-gen-btn"
                disabled={genLoading || (!form.title && !form.locationName)}
            ...
            {genSuggestion && (
              <div className="gen-suggestion-bar">
                <div style={{ marginBottom: 6, display: "flex", gap: 8, alignItems: "center" }}>
                  <strong>
                    {genSuggestion.mood} {genSuggestion.title}
                  </strong>
                </div>
                <div style={{ fontSize: 12, lineHeight: 1.7 }}>{genSuggestion.notes}</div>
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button className="btn btn-sm" onClick={acceptSuggestion}>
                    Use this ✓
                  </button>
                  <button
                    className="btn-ghost"
                    style={{ fontSize: 11 }}
                    onClick={() => setGenSuggestion(null)}
                  >
                    Discard
                  </button>
                </div>
            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="grid-2">
                <div>
                  <div className="card-title-small">Date</div>
                  <input
                    type="date"
                    className="input"
                    value={form.date}
                ...
                <div>
                  <div className="card-title-small">Mood</div>
                  <div className="mood-row">
                    {MOODS.slice(0, 6).map((m) => (
                      <button
                        key={m}
                        className={`mood-btn${form.mood === m ? " on" : ""}`}
              ...
              <textarea
                className="input"
                placeholder="Jot a few words… or tap ✨ AI Generate above to let AI write it for you"
                rows={3}
                value={form.notes}
              ...
              <div className="photo-upload-area">
                ...
                {form.photo ? (
                      ...
                      >
                        <div
                          style={{
                            width: "60%",
                            height: 2,
                            background: "linear-gradient(90deg,transparent,#4DB87A,transparent)",
                            animation: "scanLine 1.3s ease-in-out infinite",
                          }}
                        />
                        <div style={{ fontSize: 11, color: "#4DB87A", fontWeight: 600 }}>
                          Analysing photo…
                        </div>
                      </div>
                    )}
                  ...
                  <div className="photo-upload-label">
                    📷 Tap to attach a photo — Claude will caption it
                  </div>
                )}
              </div>
                          ...
                          <div className="photo-caption-body">
                            ...
                            {r.title && (
                              <div className="photo-caption-field">
                                ...
                                <div
                                  className="photo-caption-field-val"
            ...
            <div className="flex gap-8 mt-12">
              ...
              <button className="btn flex-1" onClick={handleSave} disabled={!form.title.trim()}>
                Save Memory ✨
              </button>
            </div>
          </div>
      ...
      {showStory && (
        <div
          ...
          onClick={(e) => e.target === e.currentTarget && setShowStory(false)}
        >
          <div className="story-sheet">
            ...
            {!story.loading && story.text && (
              <div className="story-footer">
                <button className="btn btn-full" onClick={generateStory}>
                  🔄 Regenerate
                </button>
              </div>
            )}
...
export const PACKING_TEMPLATES = [
  {
    ...
    emoji: "🏖️",
    name: "Beach Vacation",
    desc: "Sun, sand and sea essentials",
    items: {
  ...
  },
  {
    ...
    emoji: "⛷️",
    name: "Ski Holiday",
    desc: "On-piste and après-ski gear",
    items: {
      "📄 Documents": [
      ...
      "💊 Health": [
        "Ibuprofen / muscle rub",
        "Blister plasters",
        "Altitude sickness tablets (if high resort)",
      ],
    },
  },
  {
    ...
    desc: "Sharp, efficient and carry-on only",
    items: {
      "📄 Documents": [
        "Passport / ID",
        "Business cards",
        "Meeting agenda printouts",
  ...
  },
  {
    ...
    emoji: "🥾",
    name: "Hiking Trip",
    desc: "Day hikes to multi-day treks",
    items: {
      "📄 Documents": [
      ...
      "👕 Clothing": [
        "Moisture-wicking hiking shirt (x3)",
        "Hiking trousers",
  ...
  },
  {
    ...
    emoji: "⛺",
    name: "Camping",
    desc: "Car camping and festival essentials",
    items: {
  ...
  },
  {
    ...
    emoji: "🤿",
    name: "Dive Trip",
    desc: "Scuba & snorkelling adventure",
    items: {
      "📄 Documents": [
      ...
      "👕 Clothing": [
        "Rash guard (x2)",
        "Boardshorts / bikini (x3)",
        "Quick-dry towel",
        "Wetsuit (if not renting)",
        "Light evening wear",
        "Sandals / flip flops",
      ],
      "🧴 Toiletries": [
      ...
      "💊 Health": [
        ...
        "Antihistamines",
        "Sunburn relief",
        "Diarrhoea tablets (travel gut issues)",
      ],
    },
  },
  {
    ...
    emoji: "🎒",
    name: "Backpacking",
    desc: "Long-haul budget travel, light and flexible",
    items: {
      "📄 Documents": [
      ...
      "👕 Clothing": [
        "T-shirts (x3 — quick dry)",
        "Merino wool long-sleeve",
        "Lightweight trousers (x2)",
        "Shorts",
        "Underwear (x5 — merino)",
        "Socks (x4)",
      ...
      "🧴 Toiletries": [
        ...
        "Wet wipes",
        "Microfibre towel",
        "Menstrual products / travel toiletries",
        "Laundry sheets",
      ],
      "🎒 Essentials": [
        "Backpack (40–50L)",
      ...
      "🔌 Electronics": [
        ...
        "Power bank (20000mAh)",
        "SIM unlock phone / international eSIM",
  ...
  },
  {
    ...
    emoji: "🦁",
    name: "Safari",
    desc: "Wildlife and bush adventure",
    items: {
      "📄 Documents": [
        "Passport",
        "Visa",
      ...
      "👕 Clothing": [
        "Neutral / khaki shirts (x4 — no white)",
        "Long-sleeve shirts (sun + insects)",
        "Lightweight trousers (x2)",
        "Shorts",
        "Warm fleece (cold mornings)",
        "Windproof jacket",
      ...
      "💊 Health": [
        ...
        "DEET repellent",
        "Antihistamines",
        "Antibiotic prescription (bush emergency)",
        "Imodium / rehydration",
        "Water purification tablets",
      ],
    },
  },
  {
    ...
    emoji: "🚢",
    name: "Cruise",
    desc: "Ocean liner comfort and port days",
    items: {
      "📄 Documents": [
        "Passport",
        "Cruise booking confirmation + luggage tags",
        "Travel insurance",
        "Loyalty card / sea pass",
        "Visa for ports",
        "Credit card (no foreign fees)",
      ],
      "👕 Clothing": [
        "Smart casual eveningwear (x3)",
        "Formal night outfit",
        "Daywear for ports (x4)",
        "Swimsuit (x2)",
        "Cover-up / sarong",
        "Comfortable walking shoes for ports",
        "Formal shoes",
        "Sandals",
        "Light cardigan (ship AC is strong)",
        "Sun hat",
      ],
      "🧴 Toiletries": [
        "Sunscreen",
        ...
        "Magnets (cabin organisation hack)",
        "Power strip (no surge protection required)",
      ],
      "🎒 Essentials": [
        "Small day bag for port excursions",
        "Lanyard for sea pass card",
        "Refillable water bottle",
        "Tote bag for shopping",
        "Snorkel (for beach stops)",
        "Tide chart / port maps",
      ],
      "🔌 Electronics": [
        "Power bank",
        "Camera + underwater case",
        "Laptop or tablet",
        "Adapter (ship sockets vary)",
        "Earphones",
      ],
      "💊 Health": [
        "Seasickness tablets",
  ...
  },
  {
    ...
    desc: "Destination weddings and black-tie travel",
    items: {
      "📄 Documents": [
        "Passport",
      ...
      "👕 Clothing": [
        "Main formal outfit (suit / dress)",
        "Backup smart outfit",
        "Ceremony shoes",
        "Dancing shoes / flats",
        "Accessories (jewellery, tie, cufflinks)",
        "Stain remover pen",
        "Garment bag",
        "Sewing kit (emergency repairs)",
        "Light cover-up for travel",
        "Casual change of clothes",
      ],
      "🧴 Toiletries": [
        "Full grooming kit",
      ...
      "🎒 Essentials": [
        "Gift / gift card",
        "Card",
        "Confetti (check if allowed)",
        "Small clutch / evening bag",
        "Steamer / wrinkle spray for outfit",
      ],
      "🔌 Electronics": [
