...
export function ExploreTab({ trip, itinerary, weatherCity, setWeatherCity, weatherData, weatherLoading, fetchWeather, currFrom, setCurrFrom, currTo, setCurrTo, currAmount, setCurrAmount, currResult, convertCurrency, aiQuery, setAiQuery, aiResponse, aiLoading, getAiTip }) {
  ...
  const fetchDiscovery = async (forceRefresh = false) => {
    ...
    try {
      const data = { content: [{ text: await callAi({ task: 'tip', system: `You are a hyper-local travel guide. Based on context, suggest exactly 3 things to do RIGHT NOW. Return ONLY valid JSON — no markdown.
...
- Return ONLY the JSON.`, messages: [{ role:"user", content:
...
Suggest 3 things to do right now.` }], maxTokens: 900 }) }] };
  ...
  const checkVisa = async () => {
    ...
    try {
      const data = { content: [{ text: await callAi({ task: 'tip', system: `You are a visa and travel requirements expert. Return ONLY valid JSON — no markdown, no extra text.
...
- Never invent specific URLs — only describe where to apply.`, messages: [{ role:"user", content:
            `Passport nationality: ${passport}
Destination country: ${dest}
Provide the current visa and entry requirements.` }], maxTokens: 1200 }) }] };
