...
export function PackTab({ packing, togglePacking, addPackingItem, packingTotal, packingDone, trip, itinerary }) {
  ...
  const runRecheck = async () => {
    ...
    try {
      ...
      // 4. Claude analysis
      const data = { content: [{ text: await callAi({ task: 'tip', system: `You are a meticulous travel packing advisor. Compare the traveller's packing list against the actual weather forecast and return ONLY valid JSON — no markdown, no extra text.
...
Analyse this and return the JSON re-check report.`
  ...
  const generateSuggestions = async () => {
    ...
    try {
      const data = { content: [{ text: await callAi({ task: 'tip', system: `You are a world-class travel packing expert. Analyze the trip details and return ONLY valid JSON — no markdown, no extra text.
...
- Return ONLY the JSON object.`, messages: [{ role: "user", content: "" }], maxTokens: 1200 }) }] };
      const raw = data.content?.[0]?.text || "{}";
