...
export function DocsTab({
  ...
  const allActivities = itinerary.flatMap((d) =>
    d.activities.map((a) => ({ ...a, dayLabel: d.label, dayId: d.id })),
  )
  ...
  return (
    <div>
      ...
      <div className="export-card">
        <div className="card-title" style={{ marginBottom: 4, color: "var(--amber)" }}>
          📄 Export Trip
        </div>
        <div style={{ fontSize: 12, color: "var(--text-dim)", marginBottom: 8 }}>
          Generate a print-ready PDF — itinerary, expenses, memories and packing list all in one
          page.
        </div>
        <div className="export-includes">
          {[
            ...
            <div key={label} className="export-include-chip">
              {icon} {label} <span style={{ color: "var(--amber)", fontWeight: 600 }}>{count}</span>
            </div>
          ))}
        </div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 12 }}>
          ...
          <button className="btn btn-sm" onClick={() => setAddingDoc(true)}>
            + Upload
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 12 }}>
          <div
            className={`doc-cat-chip${filterCat === "all" ? " on" : ""}`}
          ...
          {DOC_CATS.filter((c) => catCounts[c.id]).map((c) => (
            <div
              key={c.id}
              className={`doc-cat-chip${filterCat === c.id ? " on" : ""}`}
        ...
        <div className="card" style={{ border: "1px solid rgba(255,168,40,0.2)" }}>
          ...
          <div className="flex flex-col gap-8">
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }}>
                {DOC_CATS.map((c) => (
                  <div
                    key={c.id}
                    className={`doc-cat-chip${newDocForm.cat === c.id ? " on" : ""}`}
            ...
            <div>
              ...
              <select
                className="input"
                value={newDocForm.linkedTo}
                onChange={(e) =>
                  setNewDocForm((f) => ({ ...f, linkedTo: e.target.value, linkedId: "" }))
                }
              >
            ...
            <div className="attach-upload-zone" onClick={() => fileRef.current?.click()}>
              ...
              <div className="attach-upload-label">
                ...
                <span>
                  Tap to pick files
                  <br />
                  <span style={{ fontSize: 10 }}>Images, PDFs, docs — up to 10 MB each</span>
                </span>
              </div>
            </div>

            <button className="btn-ghost" onClick={() => setAddingDoc(false)}>
              Cancel
            </button>
          </div>
        </div>
              ...
              <div className="doc-vault-hdr">
                ...
                <div className="doc-vault-info">
                  ...
                  <div className="doc-vault-meta">
                    ...
                    {linkedLabel && (
                      <span style={{ color: "#5BA4FF", marginLeft: 6 }}>🔗 {linkedLabel}</span>
                    )}
                  </div>
                </div>
                <button
